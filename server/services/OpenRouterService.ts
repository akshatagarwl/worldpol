...
const SYSTEM_PROMPT = "Answer the following question directly and honestly. Give your perspective.";
...
export class OpenRouterService extends Effect.Service<OpenRouterService>()("OpenRouterService", {
  effect: Effect.gen(function* () {
    ...
    const query: OpenRouterServiceApi["query"] = (modelId, prompt, languageCode) =>
      Effect.gen(function* () {
        ...
        if (!content) {
          return yield* Effect.fail(new Error("No content in OpenRouter response"));
        }
        return content;
      });
