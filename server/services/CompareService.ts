...
import { getModel } from "../data/models.ts"
...
export class CompareService extends Effect.Service<CompareService>()(
  "CompareService",
  {
        ...
        Effect.gen(function* () {
          ...
          if (!language) {
            return yield* Effect.fail(
              new Error(`Unknown language: ${languageCode}`),
            )
          }
          ...
          const responseText = yield* router.query(
            modelId,
            topic.prompt_template,
            languageCode,
          )

          const timestamp = yield* cache.store(
            topicId,
            modelId,
            languageCode,
            prompt,
